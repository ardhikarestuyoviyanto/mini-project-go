...
type repositoryReportAdmin struct {
	DB *gorm.DB
}

func (r *repositoryReportAdmin) GetRangeDay(user_id int, start string, finish string) []model.APIResponseRekapAbsen {
...
func (r *repositoryReportAdmin) GetBulanan(user_id int, month int) []model.APIResponseRekapAbsen {
	var rekapabsen []model.APIResponseRekapAbsen
...
func NewReportAdminRepository(db *gorm.DB) domain.AdapterAdminReportRepository {
	return &repositoryReportAdmin{
		DB: db,
	}
}
